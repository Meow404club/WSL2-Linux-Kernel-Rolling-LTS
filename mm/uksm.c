...
again:
	...
	while (!get_page_unless_zero(page)) {
		 ...
		 */
		if (!folio_test_swapcache(page_folio(page)))
			goto stale;
		cpu_relax();
	}
...
void uksm_remove_vma(struct vm_area_struct *vma, const char *where)
{
	...
	if (!slot) {
		pr_warn("==> uksm_remove_vma: no slot, vma = %p\n", vma);
		goto out;
	}
...
{
	pgoff_t pgoff = page_folio(page)->index;
			      ...
			      pte_t *orig_pte, pte_t *old_pte)
{
	...
	pvmw.address = page_address_in_vma(folio, &folio->page, vma);
			...
			struct page *kpage, pte_t orig_pte)
{
	...
	addr = page_address_in_vma(page_folio(page), page, vma);
	if (addr == -EFAULT)
		goto out;
				      ...
				      struct page *kpage, u32 hash)
{
	...
	if (!PageAnon(page) || !folio_test_ksm(page_folio(kpage)))
		goto out_unlock;
				  ...
				  int *success1, int *success2)
{
	...
	*success2 = 0;
	...
	if (!PageAnon(*kpage) || !folio_test_ksm(page_folio(*kpage)))
		goto failed;
			...
			struct mm_walk *walk)
{
	...
	ret = page && folio_test_ksm(page_folio(page));
...
static struct rmap_item *get_next_rmap_item(struct vma_slot *slot, u32 *hash)
{
	...
	folio = folio_walk_start(&fw, slot->vma, addr, 0);
	if (folio) {
...
static noinline void scan_vma_one_page(struct vma_slot *slot)
{
	...
	if (folio_test_ksm(page_folio(rmap_item->page)) && in_stable_tree(rmap_item))
		goto out2;
...
void rmap_walk_ksm(struct folio *folio, struct rmap_walk_control *rwc)
{
	...
	VM_BUG_ON_PAGE(!folio_test_ksm(folio), &folio->page);
