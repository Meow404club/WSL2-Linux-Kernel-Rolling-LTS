 ...
 *     truth for every folio's TRACKED/TYPE/ZONE/GEN/ISOLATED identity
 *     AND freshness (state.h's byte-layout block); install / del /
 *     aging are one byte write (plus an L1/L2 bitmap update on gen
 *     changes) with no allocation in any fault-path operation
 ...
 *     encoded in the per-PFN state byte, advanced by global install
 *     cadence (marie_folio_install counts installs onto the head gen
 *     and advances at marie_gen_growth_threshold(type))
 ...
 *
 *   marie_state[]        — global per-PFN identity+freshness byte array
 *                          (TRACKED, TYPE, ZONE, GEN, ISOLATED; state.{h,c})
 *   marie_track_bm[type][zone][gen] — exact L1/L2/refcounted bitmap the
 *                          reclaim scan uses to find real candidates in
 *                          O(occupancy); see bitmap.h
 ...
 * bumps the global counters (marie_nr_folios + the node-global
 ...
 * dispatch.
 *
 * Walker promote-on-access is similarly synchronous: when a young PTE
 * references a tracked folio, the walker calls marie_state_inc_tier ->
 * marie_state_move_to_gen directly on marie_state[pfn] (no queueing).
 */
 ...
 * marie_gen_installs), the global marie_track_bm
 * exact bitmap, and the MARIE_PFN_NR_GENS / MARIE_ISOLATE_BATCH
 * constants live in state.{h,c}. Runtime-tunable
...
/*
 * ---------------------------------------------------------------------
 *  generation lifecycle (helpers in mm/lru_marie/state_core.c and
 *  state_reclaim.c)
 * ---------------------------------------------------------------------
 *
 * Marie keeps a cycling per-type gen ring of MARIE_PFN_NR_GENS (= 8)
 * slots, encoded in the per-PFN state byte's GEN field. Aging is driven
 * by three signals:
 *
 ...
 *     putback re-installs a hot survivor at the head gen, a cold one at
 *     oldest+1 (see marie_state_shrink_lruvec's putback loop).
 ...
 *     checks marie_gen_installs[type] against marie_gen_growth_threshold(type);
 ...
 * SIMD walker (mm/lru_marie/walker.c) clears young PTEs and calls
 * marie_state_inc_tier on tracked folios, which promotes straight to the
 * head gen on any access (see state.h's byte-layout block) via
 * marie_state_move_to_gen (no pending queue). The rmap path
 ...
 * zone + head_gen), global track bitmap, global counters (marie_nr_folios
...
bool lru_marie_add_folio(struct lruvec *lv, struct folio *folio, bool reclaiming)
{
	 ...
	 * folio is evictable, so it MUST become Marie-tracked -- the reader
	 * (lru_marie_zone_size_read) trusts the Marie counter alone for
	 * evictable buckets and will not size any folio that escapes to the
	 * legacy list. Desktop/global-only: there is no per-memcg carrier to
	 * be missing, so every evictable folio installs unconditionally.
	 *
	...
	if (folio_test_large(folio)) {
		...
		scoped_guard(marie_type_lock, &marie_type_locks[type])
			ok = marie_folio_install(folio);
		return ok;
	}

	return marie_folio_install(folio);
}
 ...
 * @new_folio. The original head install +N covered the
			 ...
			 struct folio *new_folio)
{
	...
	scoped_guard(marie_type_lock, &marie_type_locks[folio_is_file_lru(head)]) {
		...
		new_b = marie_pfn_publish_inherit(new_folio, type, head_gen,
						  zone, &old_b);
		/*
		 * nr = 0: the folio COUNT grows by one here, but the page
		 * weight must not. The parent's install already credited every
		 * page of the pre-split compound, and each sub-folio's eventual
		 * debit subtracts its own folio_nr_pages, so the page-weighted
		 * sum balances without a credit here (see the accounting note
		 * in this function's header). The +1 on marie_nr_folios is
		 * still derived from the publish transition rather than stated,
		 * so it cannot be applied twice. See account.h.
		 */
		marie_acct_settle_locked(lv, 0, old_b, new_b);
	}
}
EXPORT_SYMBOL_GPL(lru_marie_split_folio);

/*
 * lru_marie_split_folio_isolated - reclaim-split counterpart of
 * lru_marie_split_folio. See the header doc in <linux/lru_marie.h>.
 *
 * Symmetry with the on-LRU split is the whole point: lru_add_split_folio()'s
 * on-LRU branch (head still on the LRU) routes tails through
 * lru_marie_split_folio + folio_set_lru, so a normal split keeps every tail
 ...
 */
void lru_marie_split_folio_isolated(struct folio *head, struct folio *new_folio)
{
	if (!folio_marie_test_tracked(head))
		return;
	if (folio_test_unevictable(head))
	...
	scoped_guard(marie_type_lock, &marie_type_locks[folio_is_file_lru(head)]) {
		 ...
		 * no scan bit, no gen_occupied, no nr_folios), NOT a fresh
		...
		marie_pfn_publish_isolated(new_folio, type, head_gen, zone,
					   &old_b);
		/*
		 * No settle call: the tail enters the ISOLATED state, so pred
		 * is 0 on both sides of the transition and the derived delta is
		 * 0 -- which is exactly this function's contract (no scan bit,
		 * no gen_occupied, no nr_folios). The tail is balanced exactly
		 * once downstream, by its free or by the survivor putback.
		 */
	}
}
EXPORT_SYMBOL_GPL(lru_marie_split_folio_isolated);
...
bool lru_marie_del_folio(struct lruvec *lv, struct folio *folio, bool reclaiming)
{
	 ...
	 * under Marie's install helpers (marie_folio_install on every install
	 * path, marie_pfn_publish_inherit on split's tail,
	 * marie_pfn_publish_isolated on a reclaim-split tail -- see
	 * pfn_install.h) and only ever cleared by Marie's evict paths. A
	 * survivor putback (state.c) leaves TRACKED untouched throughout, so
	 * it is not a "set" site in this list. A folio on a Marie self-loop
	 * must be handled by Marie; falling through to legacy lruvec_del_folio
	 * would list_del(&folio->lru) on the self-loop and corrupt the folio.
	 */
	 ...
	 * caller's lru_lock).
	 *
	 * marie_del_folio_locked -> marie_evict_locked -> marie_state_drop_pfn ->
	 * marie_acct_settle_locked
	 ...
	 */
	return marie_del_folio_locked(folio);
}
			     ...
			     unsigned long *flagsp)
{
	...
	/*
	 * lru_marie_del_folio re-tests TRACKED under the lock. A Marie folio
	 ...
	 * the folio was never Marie-tracked -- the not-tracked legacy
	 * fall-through path.
	 */
	if (lru_marie_del_folio(*lruvecp, folio, false)) {
		 ...
		 * ordering -- see its body). The stock __folio_clear_lru_flags()
		 * would re-assert PG_lru is still set (VM_BUG_ON_FOLIO under
	...
	/*
	 * Not Marie-tracked: this folio lives on a legacy lruvec list with
 ...
 * which memcg a folio is charged to. cgroup charging (memory.current, PSI)
 ...
 *
 * Contract: while Marie is enabled, EVERY evictable folio is Marie-tracked
 * (lru_marie_add_folio admits all evictable folios; the only non-Marie folios
 ...
 *   - LRU_UNEVICTABLE                 -> @stock (unevictable folios live on
 *                                        the legacy list, counted in stock).
 *   - Marie disabled                  -> @stock (legacy / MGLRU regime).
 *
 * The contract is checked, not merely asserted: lru_marie_add_folio WARNs if
 * an evictable folio is ever turned away to legacy, and the
 ...
 * before the cgroup tree is populated and before any lruvec carries folios, so
...
/*
 * boot param: lru_marie_max_l2_pages_per_bit=<N>. Overrides
 * MARIE_MAX_L2_PAGES_PER_BIT_DEFAULT (bitmap.h) for marie_track_bm's L2
 * summary granularity. Boot-only, not sysfs: marie_bm_global_init()
 * (marie_state_init(), subsys_initcall) consumes marie_max_l2_pages_per_bit
 * to size the L2/range-lock arrays before any folio is ever tracked, and
 * resizing them live is not supported. A value of 0 (or an unparsable
 * string) is ignored, leaving the compile-time default in effect.
 */
static int __init marie_max_l2_pages_per_bit_setup(char *str)
{
	unsigned int v;

	if (!str || kstrtouint(str, 0, &v) || !v)
		return 0;
	marie_max_l2_pages_per_bit = v;
	return 1;
}
__setup("lru_marie_max_l2_pages_per_bit=", marie_max_l2_pages_per_bit_setup);
 ...
 * (before any folio is tracked). Runtime toggling was removed with the whole
...
{
	/*
	 * orphan_bit: self-heal count for a stale L1-bitmap-vs-state-byte
	 * mismatch found by the scanner (marie_state_isolate_scan_l2lock's
	 * orphaned-bit cleanup). Restored alongside the exact L1/L2 bitmap
	 * (see state.h's paradigm-spec block): this is the same live
	 * observability signal that first surfaced the walker-vs-isolate
	 * race this whole ISOLATED-gate effort exists to close. With the
	 * gate in place this should stay near zero, bounded to the narrow
	 * residual race the gate cannot close (see
	 * marie_state_move_to_gen's comment) -- a sustained climb is a
	 * regression signal worth investigating, not routine noise.
	 */
	int len = 0, type, i;

	/*
	 * gen_ring: the aging ring itself, printed oldest-first from the slot
	 * after the head so the reader sees it in the order reclaim consumes
	 * it, with the head last and marked. Without this the ring is the one
	 * part of Marie with no external view at all: whether it has
	 * stratified (several occupied slots) or degenerated (everything in
	 * one) decides whether promote-on-access can protect anything, and
	 * that question has been argued from arithmetic rather than looked at.
	 */
	for (type = 0; type < 2; type++) {
		int head = atomic_read(&marie_head_gen[type]);

		len += sysfs_emit_at(buf, len, "gen_ring %s head %d gens %d occ",
				     type ? "file" : "anon", head,
				     atomic_read(&marie_nr_occupied_gens[type]));
		for (i = 1; i <= MARIE_PFN_NR_GENS; i++) {
			int slot = (head + i) & (MARIE_PFN_NR_GENS - 1);

			len += sysfs_emit_at(buf, len, " %d:%ld%s", slot,
				atomic_long_read(&marie_gen_occupied[slot][type]),
				slot == head ? "(H)" : "");
		}
	...
	return len + sysfs_emit_at(buf, len,
			  ...
			  "gen_growth_live anon %lu file %lu floor_shift %u\n"
			  "second_chance anon activate %ld ref_keep %ld\n"
			  "second_chance file activate %ld ref_keep %ld\n"
			  "budget need %ld delivered %ld calls %ld max_overshoot %ld\n"
			  ...
			  marie_gen_growth_threshold(0),
			  marie_gen_growth_threshold(1),
			  READ_ONCE(marie_gen_floor_shift),
			  atomic_long_read(&marie_dbg_second_chance[0][0]),
			  atomic_long_read(&marie_dbg_second_chance[1][0]),
			  atomic_long_read(&marie_dbg_second_chance[0][1]),
			  atomic_long_read(&marie_dbg_second_chance[1][1]),
			  atomic_long_read(&marie_dbg_budget[0]),
			  atomic_long_read(&marie_dbg_budget[1]),
			  atomic_long_read(&marie_dbg_budget[2]),
			  atomic_long_read(&marie_dbg_budget[3]),
...
/*
 * gen_floor_shift -- extra right-shift on the install-cadence floor, 0..6,
 * default 0.
 *
 * The cadence is max(base >> (NR_GENS + 1 - gens), base >> (3 + shift)), so the
 * two meet at gens == 3 - shift + ... -- concretely, at gens == ilog2(NR_GENS)
 * minus this value. The knob therefore says one thing: the occupied-generation
 * count from which the dynamic schedule is allowed to act.
 *
 *   0 (default) schedule acts from gens >= 6: a brake near saturation only,
 *               leaving the cadence below that at one generation per 1/NR_GENS
 *               of what the type could hold. This is the arm that measured
 *               clean.
 *   6 (cap)     floor drops below the schedule's own minimum, so the schedule is
 *               unfloored and free to accelerate while the ring is empty. This
 *               is the arm that regressed: the head lapped before reclaim could
 *               drain behind it and jammed in 3 of 3 runs against 0 of 3, with
 *               11% more pages swapped.
 *
 * In between is unmeasured. The regime it governs -- a ring at or near
 * saturation -- has not been reproducible in QEMU, while live hardware reaches
 * it in about twenty minutes, so the knob is here to be swept there. It should
 * be removed once a value is established, the same way the ignore_refs selector
 * was once its own question was answered.
 */
unsigned int marie_gen_floor_shift;

static ssize_t gen_floor_shift_show(struct kobject *kobj,
				    struct kobj_attribute *attr, char *buf)
{
	return sysfs_emit(buf, "%u\n", READ_ONCE(marie_gen_floor_shift));
}

static ssize_t gen_floor_shift_store(struct kobject *kobj,
				     struct kobj_attribute *attr,
				     const char *buf, size_t count)
{
	unsigned int v;
	...
	if (err)
		return err;
	if (v > MARIE_GEN_FLOOR_SHIFT_MAX)
		return -EINVAL;
	WRITE_ONCE(marie_gen_floor_shift, v);
	return count;
}

static struct kobj_attribute marie_gen_floor_shift_attr =
	__ATTR_RW(gen_floor_shift);
...
static struct attribute *marie_attrs[] = {
	...
	&marie_gen_floor_shift_attr.attr,
#ifdef CONFIG_SWAP
